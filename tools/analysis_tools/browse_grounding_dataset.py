...
def parse_args():
    ...
    parser.add_argument('--show-interval', type=float, default=0, help='the interval of show (s)')
    parser.add_argument(
        '--cfg-options',
        nargs='+',
        action=DictAction,
    ...
    for i in dataset_index[: args.show_num]:
        ...
        if dataset_mode == 'VG':
            ...
            for label, bbox, color in zip(gt_labels, gt_bboxes, colors):
                visualizer.draw_bboxes(bbox, edge_colors=color, face_colors=color, alpha=0.3)
            ...
            for label in gt_labels:
                label_names.append(text[tokens_positive[label][0][0] : tokens_positive[label][0][1]])
            gt_instances.label_names = label_names
            data_sample.gt_instances = gt_instances

            visualizer.add_datasample(
                base_name, img, data_sample, draw_pred=False, show=False, wait_time=0, out_file=None
            )
            drawn_img = visualizer.get_image()
