...
def get_regions(nc, anno):
    ...
    if not isinstance(nc, list):
        ...
        if not all(0 <= bbox[i] <= w-1 if i % 2 == 0 else 0 <= bbox[i] <= h-1 for i in range(4)):
            return None
...
def process_item(file, args):
    ...
    for nc in repeat_noun:
        region = get_regions(nc, annotation)
...
if __name__ == "__main__":
    ...
    # json_name = os.path.join(args.output_dir, f'girt_14m_odvg_all.json')
