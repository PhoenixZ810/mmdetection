...
def check_for_positive_overflow(gt_bboxes, gt_labels, text, tokenizer, max_tokens):
    ...
    return gt_bboxes[keep_box_index], np.array(keep_gt_labels, dtype=np.longlong), length, keep_box_index
...
class RandomSamplingNegPos(BaseTransform):
    ...
    def od_aug(self, results):
        ...
        gt_bboxes, gt_labels, positive_caption_length, keep_box_index = check_for_positive_overflow(
    ...
    def ref_aug(self, results):
        ...
        if isinstance(gt_bboxes, BaseBoxes):
            gt_bboxes = gt_bboxes.tensor
        gt_labels = results['gt_bboxes_labels']
        # save original positive phrases, when phrase is list ,save all of them
        original_phrases_list = []
        for phrase in results['text'].values():
            if isinstance(phrase, list):
                original_phrases_list.extend(phrase)
            else:
                original_phrases_list.append(phrase)
        original_phrases_list = list(set(original_phrases_list))
        # rebuild text&phrases, when phrase is list, random select one of them
        for i in results['phrases'].keys():
            if isinstance(results['phrases'][i]['phrase'], list):
                p = random.random()
                if p < 0.5:
                    results['phrases'][i]['phrase'] = random.choice(results['phrases'][i]['phrase'])
                else:
                    results['phrases'][i]['phrase'] = '. '.join(results['phrases'][i]['phrase'])
        results['text'] = {str(i): results['phrases'][i]['phrase'] for i in results['phrases'].keys()}
        text = results['text']
        ...
        if num_negatives != -1:
            ...
            # if negative phrase is list, choose one of them
            neg_list = random.sample(dataset.ref_phrases, len(positive_phrase_list) + num_negatives)
            for i in range(len(neg_list)):
                if isinstance(neg_list[i], list):
                    neg_list[i] = random.choice((neg_list[i]))
            neg_list = list(set(neg_list))
            # if negative phrase not in original positive phrases, add it
            for i in range(len(neg_list)):
                neg = neg_list[i]
                if neg not in original_phrases_list:
                    negative_phrase_list.add(neg)
                if len(negative_phrase_list) >= num_negatives:
                    break
        ...
        for negative_phrase in negative_phrase_list:
            ...
            elif screened_negative_phrase_list == []:
                continue
            else:
                break
