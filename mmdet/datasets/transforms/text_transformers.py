...
def check_for_positive_overflow(gt_bboxes, gt_labels, text, tokenizer, max_tokens, sentence=None):
    ...
    if sentence == None:
        ...
        length = len(tokenized)
        if length > max_tokens:
            print('warning: sentence length is too long!')
...
class RandomSamplingNegPos(BaseTransform):
    def __init__(self, tokenizer_name, num_sample_negative=85, max_tokens=256, full_sampling_prob=0.5):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'mixed_sentence' not in results:  # OD data
            ...
            check_for_positive_overflow(
                ...
                self.tokenizer,
                self.max_tokens,
                sentence=results['mixed_sentence'],
            )
            results['gt_bboxes'] = gt_bboxes
            results['gt_bboxes_labels'] = gt_labels
