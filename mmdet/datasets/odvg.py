...
class ODVGDataset(BaseDetDataset):
    ...
    def __init__(self, *args, data_root: str = '', label_map_file: Optional[str] = None, **kwargs) -> None:
    ...
    def load_data_list(self) -> List[dict]:
        ...
        for data in data_list:
            ...
            elif self.dataset_mode is 'VG':
                anno = data["grounding"]
                sentences = [anno['caption']]
                for i in range(5):
                    json = random.sample(data_list, 1)
                    if json['grounding']['caption'] not in sentence:
                        sentences.append(json['grounding']['caption'])

                instances = [obj for obj in anno["regions"]]
                bboxes = [obj["bbox"] for obj in instances]
                bbox_labels = [obj["phrase"] for obj in instances]

                uni_caption_list = list(set(bbox_labels))  # 去重
                label_map = {}
                for idx in range(len(uni_caption_list)):
                    label_map[idx] = uni_caption_list[idx]  # 生成新label map
                data_info['text'] = label_map
                data_info['mixed_sentence'] = sentences
