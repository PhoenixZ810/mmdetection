    ...
    def load_data_list(self) -> List[dict]:
        ...
        for data in data_list:
            ...
            if self.dataset_mode == 'OD':
                ...
                data_info['dataset_mode'] = self.dataset_mode
                out_data_list.append(data_info)
            elif self.dataset_mode == 'REF':
                regions = data['referring']
                phrases = []
                instances = []
                phrases = {}
                for i, region in enumerate(regions):
                    ...
                    phrase = region['phrase']
                    # tokens_positive = region['tokens_positive']
                    self.ref_phrases.append(phrase)
                    if not isinstance(bbox[0], list):
                        bbox = [bbox]
                    for box in bbox:
                        instance = {}
                        ...
                        instances.append(instance)
                data_info['text'] = {str(i): phrases[i]['phrase'] for i in phrases.keys()}
                data_info['instances'] = instances
                data_info['phrases'] = phrases
                data_info['dataset_mode'] = self.dataset_mode
                out_data_list.append(data_info)
        print(f'ignore {ignore} instances')
        del data_list
