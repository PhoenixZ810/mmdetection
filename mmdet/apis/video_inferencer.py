...
class VideoInferencer(BaseInferencer):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            raise TypeError('model must be a filepath or any ConfigType' f'object, but got {type(model)}')
    ...
    def __call__(
        ...
        **kwargs,
        ...
        inputs['inputs'] = inputs['inputs'].unsqueeze(0)
        inputs['data_samples'] = [inputs['data_samples']]
        # inputs = self.model.data_preprocessor(inputs)
        # preds = self.forward(inputs=inputs['inputs'], data_sample=inputs['data_samples'], mode='predict')
