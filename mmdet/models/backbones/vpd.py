...
class VPD(BaseModule):
    ...
    def __init__(self,
        ...
        if diffusion_checkpoint is not None:
            load_checkpoint(sd_model, diffusion_checkpoint, strict=False)  # load stable_diffusion_v1-5_pretrain_third_party.pth

        self.encoder_vq = sd_model.first_stage_model  # first_stage_model
        self.unet = UNetWrapper(sd_model.model, **unet_cfg)  # diffusion_model

        # class embeddings & text adapter
        class_embeddings = CheckpointLoader.load_checkpoint(class_embed_path)  # nyu_class_embeddings.pth
        text_dim = class_embeddings.size(-1)
        self.text_adapter = TextAdapter(text_dim=text_dim)
        self.class_embed_select = class_embed_select  # True
