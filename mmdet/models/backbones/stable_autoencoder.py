...
class FrozenAutoencoderKL(nn.Module):
    ...
    def sample(self, moments):
        mean, logvar = torch.chunk(
            moments, 2, dim=1)  #将1,8,64,64切分为两个1,4,64,64
        logvar = torch.clamp(logvar, -30.0, 20.0)  # 将logvar的值限制在-30-20内
        std = torch.exp(0.5 * logvar)
        z = mean + std * torch.randn_like(mean)  # 设置正态分布噪声相加
